...
const isValidParamValue = (val: null) => {
  const t = typeof val
...
export const param = function(o: { [x: string]: any }, serializeArray = true, sep = '&', eq = '=') {
  const buf = []
 ...
 * query字符串转为对象
 */
export const unparam = function(str: string, sep: string, eq: string) {
  str = str + ''
  sep = sep || '&'
  eq = eq || '='
